...
class DataConnectorMysql extends DataConnector
{
    ...
    #    Class constructor
    ###
    public function __construct($dbTableNamePrefix = '')
    {
    ...
    public function toolConsumerLoad($consumer)
    {
        ...
        $ok = false;
        $sql = sprintf(
            'SELECT name, secret, lti_version, consumer_name, consumer_version, consumer_guid, css_path, protected, enabled, enable_from, enable_until, last_access, created, updated ' .
    ...
    public function toolConsumerSave($consumer)
    {
        ...
        if (is_null($consumer->created)) {
            $sql = sprintf(
                "INSERT INTO {$this->dbTableNamePrefix}" . DataConnector::CONSUMER_TABLE_NAME . ' (consumer_key, name, ' .
                ...
                DataConnector::quoted($consumer->secret),
                DataConnector::quoted($consumer->lti_version),
                DataConnector::quoted($consumer->consumer_name),
                DataConnector::quoted($consumer->consumer_version),
                DataConnector::quoted($consumer->consumer_guid),
                DataConnector::quoted($consumer->css_path),
                DataConnector::quoted($from),
                DataConnector::quoted($until),
                DataConnector::quoted($last)
            );
        } else {
            $sql = sprintf(
                "UPDATE {$this->dbTableNamePrefix}" . DataConnector::CONSUMER_TABLE_NAME . ' SET ' .
    ...
    public function toolConsumerDelete($consumer)
    {
        ...
        $sql = sprintf(
            "DELETE FROM {$this->dbTableNamePrefix}" . DataConnector::NONCE_TABLE_NAME . ' WHERE consumer_key = %s',
            DataConnector::quoted($consumer->getKey())
        );
        mysql_query($sql);
        ...
        $sql = sprintf(
            "DELETE FROM {$this->dbTableNamePrefix}" . DataConnector::RESOURCE_LINK_SHARE_KEY_TABLE_NAME . ' WHERE primary_consumer_key = %s',
            DataConnector::quoted($consumer->getKey())
        );
        mysql_query($sql);
        ...
        $sql = sprintf(
            "DELETE FROM {$this->dbTableNamePrefix}" . DataConnector::USER_TABLE_NAME . ' WHERE consumer_key = %s',
            DataConnector::quoted($consumer->getKey())
        );
        mysql_query($sql);
        ...
        $sql = sprintf(
            "DELETE FROM {$this->dbTableNamePrefix}" . DataConnector::RESOURCE_LINK_TABLE_NAME . ' WHERE consumer_key = %s',
            DataConnector::quoted($consumer->getKey())
        );
        mysql_query($sql);
        ...
        // Delete consumer
        $sql = sprintf(
            "DELETE FROM {$this->dbTableNamePrefix}" . DataConnector::CONSUMER_TABLE_NAME . ' WHERE consumer_key = %s',
            DataConnector::quoted($consumer->getKey())
        );
        $ok = mysql_query($sql);
    ...
    public function resourceLinkLoad($resource_link)
    {
        ...
        $sql = sprintf(
            ...
            DataConnector::quoted($resource_link->getKey()),
            DataConnector::quoted($resource_link->getId())
        );
    ...
    public function resourceLinkSave($resource_link)
    {
        ...
        if (is_null($resource_link->created)) {
            $sql = sprintf(
                "INSERT INTO {$this->dbTableNamePrefix}" . DataConnector::RESOURCE_LINK_TABLE_NAME . ' (consumer_key, context_id, ' .
                ...
                DataConnector::quoted($key),
                DataConnector::quoted($id),
                DataConnector::quoted($resource_link->lti_context_id),
                DataConnector::quoted($resource_link->lti_resource_id),
                DataConnector::quoted($resource_link->title),
                DataConnector::quoted($resource_link->primary_consumer_key),
                DataConnector::quoted($resource_link->primary_resource_link_id)
            );
        } else if ($id == $previous_id) {
            $sql = sprintf(
                "UPDATE {$this->dbTableNamePrefix}" . DataConnector::RESOURCE_LINK_TABLE_NAME . ' SET ' .
            ...
            $sql = sprintf(
                ...
                DataConnector::quoted($resource_link->primary_resource_link_id),
                DataConnector::quoted($key),
                DataConnector::quoted($previous_id)
            );
        }
        $ok = mysql_query($sql);
        if ($ok) {
    ...
    public function resourceLinkDelete($resource_link)
    {
        ...
        if ($ok) {
            $sql = sprintf(
                "DELETE FROM {$this->dbTableNamePrefix}" . DataConnector::USER_TABLE_NAME . ' ' .
                'WHERE (consumer_key = %s) AND (context_id = %s)',
                DataConnector::quoted($resource_link->getKey()),
                DataConnector::quoted($resource_link->getId())
            );
    ...
    public function resourceLinkGetUserResultSourcedIDs($resource_link, $local_only, $id_scope)
    {
        ...
        if ($local_only) {
            $sql = sprintf(
                'SELECT u.consumer_key, u.context_id, u.user_id, u.lti_result_sourcedid ' .
    ...
    public function resourceLinkGetShares($resource_link)
    {
        ...
        $sql = sprintf(
            'SELECT consumer_key, context_id, title, share_approved ' .
    ...
    #
        $sql = sprintf(
            "SELECT value AS T FROM {$this->dbTableNamePrefix}" . DataConnector::NONCE_TABLE_NAME . ' WHERE (consumer_key = %s) AND (value = %s)',
            DataConnector::quoted($nonce->getKey()),
            DataConnector::quoted($nonce->getValue())
        );
    ...
    public function consumerNonceSave($nonce)
    {
        ...
        $sql = sprintf(
            "INSERT INTO {$this->dbTableNamePrefix}" . DataConnector::NONCE_TABLE_NAME . " (consumer_key, value, expires) VALUES (%s, %s, '{$expires}')",
            DataConnector::quoted($nonce->getKey()),
            DataConnector::quoted($nonce->getValue())
        );
        $ok = mysql_query($sql);
    ...
    public function resourceLinkShareKeySave($share_key)
    {
        ...
        $sql = sprintf(
            ...
            DataConnector::quoted($share_key->getId()),
            DataConnector::quoted($share_key->primary_consumer_key),
    ...
    public function userLoad($user)
    {
        ...
        $sql = sprintf(
            ...
            DataConnector::quoted($user->getResourceLink()->getKey()),
            DataConnector::quoted($user->getResourceLink()->getId()),
    ...
    public function userSave($user)
    {
        ...
        if (is_null($user->created)) {
            $sql = sprintf(
                "INSERT INTO {$this->dbTableNamePrefix}" . DataConnector::USER_TABLE_NAME . ' (consumer_key, context_id, ' .
            ...
            $sql = sprintf(
                "UPDATE {$this->dbTableNamePrefix}" . DataConnector::USER_TABLE_NAME . ' ' .
